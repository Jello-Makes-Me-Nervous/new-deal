...
function mainContent() {
    ...
    $header = "<h3>Dealer Active Sells - ".$dealerCnt." Dealers with ".$sellCnt." active sell listings</h3>\n";
    $i = 0;
    echo $header;
    echo "<article>\n";
    ...
    foreach ($data as $d) {
        ...
        $url  = "marketsnapshot.php?dealer=".$d['username']."&type=FS&sortby=cat&hourssince=0";
        $eliteUser = ($d['iselite']) ? " <span title='Elite Dealer'><i class='fas fa-star'></span>" : "";
        echo "      <a href='".$url."'>".$d['username']."</a> (".$d['sells'].")\n";
        if ($i % 50 != 49) {
            echo "      <br />\n";
        }
        if ($i % 50 == 49) {
...
function getSells() {
    ...
    $sql = "
        SELECT x.username, x.sells,
               CASE WHEN ar.userid IS NOT NULL THEN 1
                    ELSE 0 END as iselite
          FROM (
            SELECT u.userid, u.username, COUNT(l.listingid) AS sells
              FROM listings             l
