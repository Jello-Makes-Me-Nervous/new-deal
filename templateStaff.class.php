...
class templateStaff extends template{
    ...
    public function __construct($loginRequired = true, $showmsgs = true, $bypassMustReplyCheck = false) {
        ...
        if (!($this->user->hasUserRight('ADMIN') || $this->user->hasUserRight('STAFF'))) {
            header('location:home.php?pgemsg=The%20requested%20page%20requires%20STAFF%20access');
            exit();
        }
    }
